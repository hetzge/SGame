...
public class ReservedMap implements IF_ReserveMap {
	...
	private final Map<IF_Coordinate_Immutable, IF_MapMoveable> reservedMap = new HashMap<>();

	@Override
	public synchronized boolean isReserved(IF_Coordinate_Immutable coordinate) {
		IF_MapMoveable moveable = this.reservedMap.get(coordinate);
		return moveable != null;
	}

	@Override
	public void reserve(IF_Coordinate_Immutable coordinate, IF_MapMoveable moveable) {
		this.reservedMap.put(coordinate, moveable);
	}

	@Override
	public synchronized void unreserve(IF_Coordinate_Immutable coordinate, IF_MapMoveable moveable) {
